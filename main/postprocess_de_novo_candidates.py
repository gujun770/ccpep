import sys
from pathlib import Path

import pandas as pd

PROJECT_ROOT = Path(__file__).resolve().parent.parent
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from utils.project_paths import RESULT_DIR


def parse_tokens(token_str: str):
    if not isinstance(token_str, str):
        return []
    return [t for t in token_str.split(".") if t]


def composition_signature(tokens):
    return ".".join(sorted(tokens))


def jaccard_distance(tokens_a, tokens_b):
    set_a = set(tokens_a)
    set_b = set(tokens_b)
    union = len(set_a | set_b)
    if union == 0:
        return 0.0
    return 1.0 - (len(set_a & set_b) / union)


def build_diverse_shortlist(df, top_k=24, min_jaccard_distance=0.34):
    selected = []
    selected_tokens = []
    seen_compositions = set()

    for _, row in df.iterrows():
        tokens = row["tokens_list"]
        comp = row["composition_signature"]
        if comp in seen_compositions:
            continue

        keep = True
        for existing in selected_tokens:
            if jaccard_distance(tokens, existing) < min_jaccard_distance:
                keep = False
                break
        if keep:
            selected.append(row)
            selected_tokens.append(tokens)
            seen_compositions.add(comp)
        if len(selected) >= top_k:
            break

    return pd.DataFrame(selected)


def main():
    in_path = RESULT_DIR / "de_novo_generation" / "final_generated_candidates.csv"
    out_dir = RESULT_DIR / "de_novo_generation"

    df = pd.read_csv(in_path)
    df["tokens_list"] = df["tokens"].astype(str).apply(parse_tokens)
    df["composition_signature"] = df["tokens_list"].apply(composition_signature)

    sort_cols = [c for c in ["multiobjective_score", "robust_score", "predicted_positive_prob", "predicted_permeability"] if c in df.columns]
    ranked = df.sort_values(sort_cols, ascending=False).reset_index(drop=True)

    shortlist = build_diverse_shortlist(ranked, top_k=24, min_jaccard_distance=0.34)
    shortlist = shortlist.copy()
    shortlist.drop(columns=["tokens_list"], inplace=True, errors="ignore")
    shortlist.to_csv(out_dir / "final_generated_shortlist.csv", index=False)

    substitution_summary = (
        shortlist["tokens"].str.split(".")
        .explode()
        .value_counts()
        .reset_index()
    )
    substitution_summary.columns = ["monomer", "count"]
    substitution_summary.to_csv(out_dir / "generated_monomer_frequency.csv", index=False)

    print("Final de novo shortlist:")
    print(
        shortlist[
            [
                "HELM",
                "tokens",
                "predicted_permeability",
                "predicted_positive_prob",
                "permeability_std",
                "positive_prob_std",
                "multiobjective_score",
                "motif_score",
                "composition_alignment",
                "robust_score",
                "natural_ratio",
            ]
        ].to_string(index=False)
    )
    print("---")
    print("Generated monomer frequency:")
    print(substitution_summary.head(15).to_string(index=False))
    print(f"Saved shortlist to: {out_dir / 'final_generated_shortlist.csv'}")


if __name__ == "__main__":
    main()
