...
from utils.project_paths import HELM_DIR, resolve_dataset_path, resolve_model_path
import pandas as pd
...
class Customize(pl.LightningModule):
    def __init__(self,
                 ...
                 sigma=60,
                 weight_permeability=1.0,
                 weight_length=1.0,
                 weight_natural=1.0,
                 weight_time=0.2):
        super().__init__()
        self.tokenizer = AutoTokenizer.from_pretrained(str(HELM_DIR),
                                                       ignore_mismatched_sizes=True)
        self.restore_prior_from = resolve_model_path('Prior-RNN.ckpt')
        self.restore_agent_from = resolve_model_path('Prior-RNN.ckpt')
        self.agent_save = resolve_model_path('Len6-batch2.ckpt')
        self.sigma = sigma
        self.target_length = target_length
        self.batch_sample_size = batch_sample_size
        self.reward_weights = {
            'permeability': weight_permeability,
            'length': weight_length,
            'natural': weight_natural,
            'time': weight_time,
        }
        self.save_dir = str((HELM_DIR.parent / 'Result' / 'Len6-batch2'))
        self.dataset = DatasetHELM(path=resolve_dataset_path('pretrain.csv'))
    ...
    def training_step(self, batch, batch_idx):
        helm_save = []
        seqs, agent_likelihood, entropy = self.Agent.sample(batch_size=self.batch_sample_size)
        ...
        score_length = length_customisation(helm_ids, length=self.target_length)
        ...
        if batch_idx % 50 == 0 and batch_idx != 0:
            ...
            df.to_csv(os.path.join(self.save_dir, f'sample_{batch_idx}.csv'), index=False, header=True)
        ...
        loss = (
            self.reward_weights['permeability'] * loss_permeability +
            self.reward_weights['length'] * loss_length +
            self.reward_weights['natural'] * loss_natural +
            self.reward_weights['time'] * loss_time
        )
        loss = loss.mean()
    ...
    def train_dataloader(self):
        dataset = DatasetHELM(path=resolve_dataset_path('pretrain.csv'))
