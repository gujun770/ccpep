    ...
    def __init__(self, func):
        ...
        if func == 'Triple':
            self.tokenizer = AutoTokenizer.from_pretrained(str(HELM_DIR),
                                                           ignore_mismatched_sizes=True)
            self.Score = BartForSequenceClassification.from_pretrained(str(HELM_DIR),
                                                                       ignore_mismatched_sizes=True)
            self.Score.resize_token_embeddings(len(self.tokenizer))
            self.Score.load_state_dict(torch.load(resolve_model_path('Score_Function', 'Triple', 'Triple-Bart-HELM-fold1.ckpt')))
        ...
        if func == 'Binary':
            ...
            self.Score = BartForSequenceClassification.from_pretrained(str(HELM_DIR),
                                                                       ignore_mismatched_sizes=True)
            self.Score.resize_token_embeddings(len(self.tokenizer))
            self.Score.load_state_dict(torch.load(resolve_model_path('Score_Function', 'Binary', 'Binary-Bart-HELM-fold1.ckpt')))
        ...
        if func == 'Regression':
            ...
            self.Score = BartForSequenceClassification.from_pretrained(str(HELM_DIR),
                                                                       ignore_mismatched_sizes=True)
            self.Score.resize_token_embeddings(len(self.tokenizer))
            self.Score.load_state_dict(torch.load(resolve_model_path('Score_Function', 'Regression', 'Regression-Bart-HELM-fold1.ckpt')))
...
class ScoreTime(nn.Module):
    def __init__(self):
        ...
        self.helm_enc.load_state_dict(torch.load(resolve_model_path('helm_enc_final.pth')))
        self.classify_h.load_state_dict(torch.load(resolve_model_path('helm_project_final.pth')))
